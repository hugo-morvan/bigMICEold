...
# OLD IMPLEMENTATION, not optimal, creates a bunch of temporary tables
impute_with_linear_regression_SQL <- function(sc, sdf, target_col, feature_cols) {
  # Given a spark connection, a spark dataframe, a target column with missing values,
  # and feature columns without missing values, this function:
  # 1. Builds a linear regression model using complete cases
  # 2. Uses that model to predict missing values
...
#Call the imputer
imputed_missing <- impute_with_linear_regression(sc, df_missing, label_col, features_col)
imputed_missing <- impute_with_linear_regression(sc, df_missing, label_col, features_col, elastic_net_param = 1)
